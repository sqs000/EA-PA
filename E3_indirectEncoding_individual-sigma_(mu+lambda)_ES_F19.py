...
def initialization(mu):
    ...
    initial_sigma = []
    for i in range(mu):
        initial_sigma.append([0.05 * (31 - 0)]*int(dimension/5))
...
def mutation(population, sigma, tau_prime, tau):
    mutated_population = np.copy(population)
    mutated_sigma = np.copy(sigma)
    global_pertubation = np.random.normal(0, tau_prime)
    for individual, individual_sigma in zip(mutated_population, mutated_sigma):
        for i in range(len(individual)):
            individual_sigma[i] = individual_sigma[i] * np.exp(global_pertubation + np.random.normal(0, tau))
            individual[i] = int(individual[i] + np.random.normal(0, individual_sigma[i]))
...
def selection(parent_pop, offspring_pop, parent_sigma, offspring_sigma, problem, mu):
    ...
    sigma = np.concatenate((parent_sigma, offspring_sigma), axis=0)
    # evaluation and sort
...
def s3674320_s3649024_ES(problem):
    ...
    tau_prime = 1.0 / np.sqrt(2*dimension/5)
    tau =  1.0 / np.sqrt(2*np.sqrt(dimension/5))
    ...
    while problem.state.evaluations < budget:
        ...
        # mutation
        mutated_pop, mutated_sigma = mutation(recombined_pop, recombined_sigma, tau_prime, tau)
        # decoding
...
def create_problem(fid: int):
    ...
    l = logger.Analyzer(
        ...
        algorithm_name="individual-sigma ES: 50+500",  # name of your algorithm
