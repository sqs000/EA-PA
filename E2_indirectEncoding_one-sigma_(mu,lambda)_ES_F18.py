...
def initialization(mu):
    ...
    initial_sigma = []
    for i in range(mu):
        initial_sigma.append(0.05 * (31 - 0))
...
# (mu, lambda) selection
def selection(offspring_pop, offspring_sigma, problem, mu):
    # evaluation and sort
    fitness_values = [problem(x) for x in offspring_pop]
    sorted_indices = np.argsort(fitness_values)[::-1]
    sorted_fitness = [fitness_values[i] for i in sorted_indices]
    sorted_population = [offspring_pop[i] for i in sorted_indices]
    sorted_sigma = [offspring_sigma[i] for i in sorted_indices]
    # select
...
def s3674320_s3649024_ES(problem):
    ...
    while problem.state.evaluations < budget:
        ...
        # selection
        selected_pop, selected_sigma, fitness_values = selection(decoded_pop, mutated_sigma, problem, mu_)
        # reset
...
def create_problem(fid: int):
    ...
    l = logger.Analyzer(
        ...
        algorithm_name="one-sigma ES: 15,200",  # name of your algorithm
